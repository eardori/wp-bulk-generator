...
const LEGACY_CREDS_CACHE = join(process.cwd(), "..", "configs", "sites-credentials.json");
const LEGACY_CONFIG_CACHE = join(process.cwd(), "..", "configs", "sites-config.json");
...
function loadReservedSlugs(): Set<string> {
  ...
  let rootCreds: SiteRecord[] = [];
  try {
    rootCreds = JSON.parse(execSync("sudo cat /root/wp-sites-credentials.json", { timeout: 5000 }).toString()) as SiteRecord[];
  } catch {
    /* local dev or missing sudo */
  }
  const existing = [
    ...rootCreds,
    ...tryReadJson<SiteRecord>(CREDS_CACHE),
    ...tryReadJson<SiteRecord>(CONFIG_CACHE),
    ...tryReadJson<SiteRecord>(LEGACY_CREDS_CACHE),
    ...tryReadJson<SiteRecord>(LEGACY_CONFIG_CACHE),
  ];
...
export async function POST(req: NextRequest) {
  ...
  const koPercent = Math.round((language_ratio?.ko || 0.8) * 100);
      ...
      try {
        ...
        for (let i = 0; i < totalBatches; i++) {
          ...
          const prompt = buildPrompt(
            niche, batchCount, i, totalCount,
            koPercent, enPercent,
            isSubdomain, base_domain, domainSuffix,
