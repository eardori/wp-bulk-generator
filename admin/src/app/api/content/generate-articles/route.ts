...
export async function POST(req: NextRequest) {
  const {
    ...
    siteConfigs,
    reviewCollection,
    offset = 0,      // 배치 시작 인덱스 (프론트엔드 분할 호출용)
    limit,           // 이번 배치에서 처리할 글 수 (없으면 전체)
    globalTotal,     // 전체 글 수 (진행 표시용)
  } = (await req.json()) as {
  ...
  type ArticleTask = { site: SiteCredential; questionIndex: number; articleVariation: number };
  ...
  // 이번 배치에서 처리할 작업만 슬라이싱
  const batchTasks = limit != null ? allTasks.slice(offset, offset + limit) : allTasks.slice(offset);
  const total = globalTotal ?? allTasks.length; // 진행 표시용 전체 수
  const PARALLEL = 3; // 동시 생성 수 (Gemini rate limit 고려)
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // 3개씩 병렬 처리: 250개 × 25s ÷ 3 = ~2100s → 배치 12개씩 = 100s/배치
      for (let i = 0; i < batchTasks.length; i += PARALLEL) {
        // 첫 병렬 묶음 이후 1초 딜레이 (rate limit 완충)
        ...
        const results = await Promise.allSettled(
          parallelSlice.map(async ({ site, questionIndex, articleVariation }, sliceIdx) => {
            ...
            // 최대 5회 재시도 — 429는 지수 백오프 (30s / 60s / 120s / 180s)
            const RETRY_DELAYS = [0, 30, 60, 120, 180]; // seconds per attempt
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
              try {
                ...
                if (!is429) break; // 429 아닌 에러는 재시도 없이 바로 실패
              }
            }
...
async function generateForSite(
  apiKey: string,
  product: ScrapedProduct,
  primaryQuestion: TargetQuestion,   // 이 글의 핵심 타겟 질문
  otherQuestions: TargetQuestion[],  // 보조 언급용 질문들
  site: SiteCredential,
  reviewCollection?: ReviewCollection,
  articleVariation = 0,              // 같은 페르소나의 몇 번째 글인지 (0-indexed)
  ...
  // Use rich review collection data if available, otherwise fall back to scraped reviews
  let reviewSection = "";
...
JSON 형식으로 응답하세요:
{
  ...
  // Build reviewImages array from usedReviewImageIndices
