...
function buildPrimaryTarget() {
    ...
    return {
        ...
        repoRoot,
        credentialsPath: process.env.PRIMARY_SERVER_CREDENTIALS_PATH ||
            process.env.CREDENTIALS_PATH ||
            join(repoRoot, "bridge-api", "data", "wp-sites-credentials.json"),
        configPath: process.env.PRIMARY_SERVER_CONFIG_PATH ||
...
function buildSecondaryTarget() {
    ...
    const repoRoot = process.env.SECONDARY_SERVER_REPO_ROOT || `/home/${user}/wp-bulk-generator`;
    return {
        ...
        repoRoot,
        credentialsPath: process.env.SECONDARY_SERVER_CREDENTIALS_PATH ||
            join(repoRoot, "bridge-api", "data", "wp-sites-credentials.json"),
        configPath: process.env.SECONDARY_SERVER_CONFIG_PATH ||
            join(repoRoot, "bridge-api", "data", "wp-sites-config.json"),
    };
}
export function getPrimaryServerTarget() {
    return buildPrimaryTarget();
}
export function getSecondaryServerTarget() {
    return buildSecondaryTarget();
}
export function getDefaultDeployTarget() {
...
export function resolveSiteTarget(site) {
    ...
    if (site.server_host) {
        ...
        const isPrimaryHost = !site.server_host ||
        ...
        if (!isPrimaryHost) {
            return {
                ...
                repoRoot: site.server_repo_root ||
                    secondary?.repoRoot ||
                    `/home/${site.server_user || secondary?.user || "ubuntu"}/wp-bulk-generator`,
                credentialsPath: site.server_credentials_path ||
                    secondary?.credentialsPath ||
                    join(site.server_repo_root ||
                        secondary?.repoRoot ||
                        `/home/${site.server_user || secondary?.user || "ubuntu"}/wp-bulk-generator`, "bridge-api", "data", "wp-sites-credentials.json"),
                configPath: site.server_config_path ||
                    secondary?.configPath ||
                    join(site.server_repo_root ||
                        secondary?.repoRoot ||
                        `/home/${site.server_user || secondary?.user || "ubuntu"}/wp-bulk-generator`, "bridge-api", "data", "wp-sites-config.json"),
            };
        }
    }
    return {
...
export function getSiteDirForTarget(site, target) {
    return site.site_dir || join(site.server_site_root || target.siteRoot, site.slug);
}
//# sourceMappingURL=server-targets.js.map