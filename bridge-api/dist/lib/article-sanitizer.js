...
const REVIEW_META_REPLACEMENTS = [
...
const EMPTY_INFO_TEXT_PATTERN = /^(?:전화|주소|지번주소|지역|영업시간|주차|예약(?:여부)?|가격대|대표메뉴|메뉴|가격|편의시설|찾아가는길)\s*[:：-]?\s*(?:정보 없음|문의 필요|확인 필요)\s*$/u;
export function sanitizeInternalReviewRefs(text) {
    let sanitized = text || "";
...
function sanitizeMetaReviewPhrases(text) {
    let sanitized = text || "";
...
function sanitizeTitleCopy(text) {
    let sanitized = text || "";
...
function normalizeLength(text, maxLength) {
...
function normalizeMetaTitle(text) {
...
export function sanitizeInternalReviewRefsInHtml(html) {
...
function isReviewKeywordTag(tag) {
    const value = (tag || "").trim();
    if (!value)
        return false;
    return REVIEW_KEYWORD_TAG_PATTERNS.some((pattern) => pattern.test(value));
}
function normalizeHealthFoodStructureInHtml(html) {
...
export function sanitizeGeneratedArticle(article) {
    return {
        ...article,
        title: sanitizeTitleCopy(sanitizeMetaReviewPhrases(sanitizeInternalReviewRefs(article.title))),
        ...
        htmlContent: normalizeHealthFoodStructureInHtml(sanitizeInternalReviewRefsInHtml(article.htmlContent)),
...
//# sourceMappingURL=article-sanitizer.js.map