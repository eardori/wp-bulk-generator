...
export async function ec2Fetch(path, options) {
    if (!EC2_AGENT_URL) {
...
export async function fetchCredentials() {
    ...
    if (!res.ok)
        throw new Error(`EC2 credentials fetch 실패: ${res.status}`);
...
export async function fetchConfigs() {
    ...
    if (!res.ok)
        throw new Error(`EC2 config fetch 실패: ${res.status}`);
    const data = await res.json();
    return data.configs || [];
}
