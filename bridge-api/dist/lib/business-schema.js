...
function collectTextLines(contentHtml) {
...
function extractAddress(contentText, lines) {
    const labeled = extractLabeledValue(lines, ["주소", "위치", "지번주소", "도로명주소"]);
    if (labeled) {
        return labeled;
    }
    const match = contentText.match(/((?:서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)[^\n,]{6,120})/);
    return match?.[1]?.trim();
}
function extractTelephone(contentText, lines) {
...
function extractPriceRange(contentText) {
...
function extractAggregateRating(contentText) {
    ...
    for (const seg of segments) {
        ...
        const reviewCountMatch = seg.match(/(?:리뷰|후기)\s*(\d{1,5})\s*개|(\d{1,5})\s*개\s*(?:리뷰|후기)/);
        if (!ratingMatch?.[1] || !reviewCountMatch)
            continue;
        ...
        if (!Number.isFinite(ratingValue) || ratingValue < 1 || ratingValue > 5)
            continue;
        if (!Number.isFinite(reviewCount) || reviewCount < 5 || reviewCount > 99_999)
            continue;
        return { ratingValue: ratingMatch[1], reviewCount };
    }
    return undefined;
}
function inferCuisine(contentText) {
...
function inferBusinessName(title, sourceName) {
    if (sourceName?.trim()) {
...
function looksLikeRestaurant(text) {
    return /(맛집|식당|레스토랑|restaurant|카페|coffee|브런치|갈비|고기|한식|일식|중식|양식|베이커리|디저트|주점|와인|bar|펍|술집)/i.test(text);
}
export function stripReviewReferenceMarkers(html) {
    return html
...
export function buildBusinessSchemaFromHtml(input) {
