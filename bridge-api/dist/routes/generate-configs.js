...
async function loadReservedSlugs() {
...
function makeUniqueSlug(base, reservedSlugs) {
...
async function callGeminiWithRetry(model, prompt, sendProgress) {
...
function buildPrompt(niche, batchCount, batchIndex, totalCount, koPercent, enPercent, isSubdomain, base_domain, domainSuffix, domainExample, domainDesc) {
    const batchNote = totalCount > BATCH_SIZE
...
function normalizeConfigs(configs, isSubdomain, base_domain, domainSuffix, reservedSlugs) {
    const adjustments = [];
    const normalized = configs.map((cfg) => {
        const rawSlug = cfg.site_slug || (cfg.domain || "").split(".")[0] || "";
        const slug = makeUniqueSlug(rawSlug, reservedSlugs);
        if (slug !== sanitizeSlug(rawSlug)) {
            adjustments.push(`${sanitizeSlug(rawSlug)} -> ${slug}`);
        }
        const nextCfg = { ...cfg, site_slug: slug };
...
// ── Route ────────────────────────────────────────────────────────────────────
export async function generateConfigsRoutes(app) {
    app.post("/generate-configs", async (req, reply) => {
        const { niche, count, language_ratio, domain_mode, base_domain, domain_suffix } = req.body;
        const isSubdomain = domain_mode === "subdomain" && !!base_domain;
        const domainSuffix = domain_suffix || ".site";
        const totalCount = Number(count) || 5;
        const langRatio = language_ratio;
        ...
        try {
            ...
            for (let i = 0; i < totalBatches; i++) {
                ...
                const prompt = buildPrompt(niche, batchCount, i, totalCount, koPercent, enPercent, isSubdomain, base_domain, domainSuffix, domainExample, domainDesc);
                let responseText;
                try {
                ...
                const parsed = repairJsonArray(responseText);
                if (parsed.length === 0) {
                    sendProgress(`⚠️ 배치 ${i + 1} 파싱 실패, 건너뜀`);
                    continue;
                }
                const normalized = normalizeConfigs(parsed, isSubdomain, base_domain, domainSuffix, reservedSlugs);
                ...
                if (normalized.adjustments.length > 0) {
                    ...
                    const extra = normalized.adjustments.length > 3
...
//# sourceMappingURL=generate-configs.js.map