...
const GEMINI_CONTENT_MODEL = process.env.GEMINI_CONTENT_MODEL || "gemini-2.5-flash-lite";
const GEMINI_CONTENT_FALLBACK_MODEL = process.env.GEMINI_CONTENT_FALLBACK_MODEL || "gemini-2.5-flash";
...
function pickCoPrimeStep(reviewCount, preferredStep) {
...
async function requestGeminiContent(apiKey, prompt, generationConfig) {
    const models = Array.from(new Set([GEMINI_CONTENT_MODEL, GEMINI_CONTENT_FALLBACK_MODEL].filter(Boolean)));
    let lastError = null;
...
function pickReviewIndices(reviewCount, articleVariation, windowSize = 30, segments = 8) {
    if (reviewCount <= 0)
        return [];
...
function buildIndexedReviews(reviews, articleVariation, windowSize = 30, segments = 8) {
...
function estimateVisibleWordCount(html) {
...
function normalizeContentPrompt(prompt) {
    return (prompt || "").replace(/\r\n/g, "\n").trim();
}
function summarizeContentPrompt(prompt, limit = 90) {
...
function normalizeRestaurantPlaceTitle(title) {
...
function buildStructuredRestaurantTitleFromAngle(placeTitle, angleLabel) {
...
function getProductSpecValue(product, aliases) {
...
function buildRestaurantFactMap(product) {
    return {
        ...
        priceRange: firstNonEmptyText(product.price, getProductSpecValue(product, ["가격대", "priceRange"])),
    };
}
function pickReaderLens(isRestaurant, articleVariation) {
    const pool = isRestaurant ? PLACE_READER_LENSES : PRODUCT_READER_LENSES;
    return pool[articleVariation % pool.length];
}
function extractFrequentReviewTerms(reviews, limit = 8) {
    ...
    for (const review of reviews) {
        ...
        for (const rawToken of tokens) {
            ...
            if (REVIEW_TERM_STOPWORDS.has(token))
                continue;
            if (/^\d+$/.test(token))
                continue;
...
function buildDirectReviewCoverageSummary(reviews, heading = "전체 리뷰 공통 패턴") {
    if (reviews.length === 0)
        return "";
    ...
    const reviewerCount = new Set(reviews.map((review) => review.reviewerName?.trim()).filter(Boolean)).size;
    const frequentTerms = extractFrequentReviewTerms(reviews, 8);
    const optionCounts = Array.from(reviews.reduce((acc, review) => {
...
function pickRepresentativeReviews(indexedReviews) {
    const picked = [];
    const seen = new Set();
    const add = (label, item) => {
        if (!item || seen.has(item.globalIndex))
            return;
...
function extractReviewImageIndicesFromHtml(html) {
...
function normalizeReviewImageIndices(raw, reviews, limit = 5) {
    ...
    for (const item of raw) {
        ...
        if (!Number.isInteger(reviewIdx) || !Number.isInteger(imageIdx))
            continue;
        if (reviewIdx < 0 || imageIdx < 0)
            continue;
        if (!reviews[reviewIdx]?.images?.[imageIdx])
            continue;
...
function mergeReviewImageIndices(...lists) {
    const merged = [];
    const seen = new Set();
...
function pickFallbackReviewImageIndices(reviews, preferredReviewIndices = [], limit = 3) {
    const result = [];
    const seen = new Set();
...
function buildReviewImagesFromIndices(reviews, usedReviewImageIndices) {
    return usedReviewImageIndices
        .map(([reviewIdx, imageIdx]) => reviews[reviewIdx]?.images?.[imageIdx])
        .filter(Boolean);
}
function normalizePersona(site) {
    const raw = (site.persona ?? {});
    const fallbackName = (typeof site.title === "string" && site.title.trim()) ||
    ...
    return {
        name: typeof raw.name === "string" && raw.name.trim()
            ? raw.name.trim()
            : fallbackName,
        age: typeof raw.age === "number" && Number.isFinite(raw.age) && raw.age > 0
            ? raw.age
            : DEFAULT_PERSONA.age,
        concern: typeof raw.concern === "string" && raw.concern.trim()
            ? raw.concern.trim()
...
function injectReviewImagePlaceholders(html, usedReviewImageIndices) {
...
function buildReviewPromptSection(reviewCollection, articleVariation = 0) {
    ...
    const windowAverageRating = Math.round((indexedReviews.reduce((sum, { review }) => sum + review.rating, 0) / indexedReviews.length) * 10) / 10;
    ...
    const optionCounts = Array.from(indexedReviews.reduce((acc, { review }) => {
        const option = review.purchaseOption?.trim();
        if (option)
            acc.set(option, (acc.get(option) || 0) + 1);
        return acc;
    ...
    const windowThemes = themes
        ...
        for (const { review } of indexedReviews) {
            if (!review.text.includes(theme.keyword))
                continue;
            count += 1;
            if (sampleTexts.length < 2)
                sampleTexts.push(truncateText(review.text, 80));
        }
...
function buildPlaceReviewPromptSection(reviews, articleVariation = 0) {
    const indexedReviews = buildIndexedReviews(reviews, articleVariation, 20, 6);
    if (indexedReviews.length === 0)
        return "";
    const representativeReviews = pickRepresentativeReviews(indexedReviews);
    let section = `\n\n## 후기 원문 참고 (팩트 확인용):\n`;
...
let aeoBusinessTypesCache = null;
function loadAeoBusinessTypes() {
    if (aeoBusinessTypesCache)
...
function buildMenuItemsJson(menuStr) {
    if (!menuStr?.trim())
        return "[]";
...
function resolveAeoSectionTitle(template, vars) {
    return template.replace(/\{\{(\w+)\}\}/g, (_, key) => vars[key] || "");
}
function autoEnhanceAeoPrompt(contentPrompt) {
...
function buildAeoPrompt(aeoConfig, persona, product, reviewsSection, promptBlock, variationHint, placeHasReviewImages, articleVariation) {
    ...
    const aeoAngleOverlays = [
        ...
        {
            label: "외국인 접대·동반 방문형",
            focusInstruction: "외국인 친구, 바이어, 해외 파트너와 함께 방문하는 시나리오에 집중하세요. 한식 경험을 트렌디한 분위기에서 즐길 수 있는 포인트, 외국어 메뉴/응대 여부, 개별 구이 경험의 매력, 사진 찍기 좋은 플레이팅 등을 강조하세요. '외국인과 함께 가기 좋은 트렌디한 분위기'가 자연스럽게 드러나도록 작성하세요.",
        },
        {
            label: "비즈니스 미팅·회식형",
            focusInstruction: "젊은 감각의 비즈니스 미팅이나 회식 자리로서의 적합성에 집중하세요. 핸한 분위기 속에서 검증된 음식을 즐길 수 있는 포인트, 룸/프라이빗 공간의 비즈니스 적합도, 코스 구성의 프레젠테이션 용이성, 예약 및 단체 수용 능력을 강조하세요. '젊은 감각의 비즈니스 회식 장소'라는 이미지가 자연스럽게 드러나도록 작성하세요.",
        },
    ];
...
- 브랜딩 강조: 외국인 친구/바이어와 함께 방문하기 좋은 트렌디한 분위기인지, 젊은 감각의 비즈니스 미팅/회식에 적합한 핸한 공간인지를 자연스럽게 녹여서 서술 (입력 정보에서 근거를 찾을 수 있을 때)
...
- 브랜딩 강조: 외국인 동반 방문 시 코스 구성이나 개별 구이 경험이 어떻게 적합한지, 비즈니스 회식 시 룸 예약/단체 배석/코스 안내 가능 여부를 자연스럽게 언급 (근거가 있을 때만)
- 입력값에 있는 정보만 사용
...
function buildContentStrategyBlock(contentStrategy, articleVariation) {
    ...
    if (selectedKeyword) {
        ...
        if (contentStrategy.keywordOptimization) {
            ...
            if (ko.titleRule)
                block += `- 제목: ${ko.titleRule}\n`;
            if (ko.introRule)
                block += `- 도입부: ${ko.introRule}\n`;
            if (ko.h2Rule)
                block += `- H2: ${ko.h2Rule}\n`;
        }
    }
    ...
    if (contentStrategy.htmlStructureRules) {
        ...
        block += `\n### 추가 HTML 구조 규칙:\n`;
        if (hr.useBulletPoints)
            block += `- ${hr.useBulletPoints}\n`;
        if (hr.endingSection)
            block += `- ${hr.endingSection}\n`;
    }
    return block;
}
async function generateForSite(apiKey, product, contentPrompt, site, reviewCollection, articleVariation = 0, siteArticleIndex = 0, aeoConfig, contentStrategy) {
...
- 사용자가 "다양한 페르소나", "각기 다른 관점"을 요구한 경우 이번 글은 아래 독자 렌즈를 우선 적용할 것:
  ...
  - 서술 각도: ${readerLens.narrativeAngle}${strategyBlock}`;
    const specsSummary = Object.keys(product.specs).length > 0
    ...
    if (reviewCollection && reviewCollection.totalCount > 0) {
        ...
        reviewSection =
            `\n\n## 제품 리뷰 (스크랩 기반)\n` +
                ...
                indexedReviews
                    .map(({ globalIndex, review }) => `[리뷰#${globalIndex}] (★${review.rating}) ${review.purchaseOption ? `[${review.purchaseOption}] ` : ""}${review.text}`)
                    .join("\n");
    }
...
⑤ 붐비는 시간대와 예약 체크 포인트`,
    ...
    const variationHint = articleVariation > 0
    ...
    if (aeoConfig) {
        ...
        const aeoPromptBlock = enhancedPrompt
            ...
            : "";
        const aeoPrompt = buildAeoPrompt(aeoConfig, persona, product, reviewsSection, aeoPromptBlock, variationHint, placeHasReviewImages, articleVariation);
        const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_CONTENT_MODEL}:generateContent?key=${apiKey}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        const faqItems = (parsed.faq || []).map((f) => ({
        ...
        const aeoUsedReviewImageIndices = mergeReviewImageIndices(normalizeReviewImageIndices(parsed.usedReviewImageIndices, product.reviews), aeoPromptIndices);
        const finalAeoReviewImageIndices = aeoUsedReviewImageIndices.length > 0
            ? aeoUsedReviewImageIndices
            : pickFallbackReviewImageIndices(product.reviews, pickReviewIndices(product.reviews.length, articleVariation, 20, 6), 3);
...
JSON 형식으로 응답하세요:
{
  ...
  "faq": [
    {"question": "질문?", "answer": "답변"}
        ...
        const restaurantUsedReviewImageIndices = mergeReviewImageIndices(normalizeReviewImageIndices(parsed.usedReviewImageIndices, product.reviews), restaurantPromptIndices);
        const finalRestaurantReviewImageIndices = restaurantUsedReviewImageIndices.length > 0
...
JSON 형식으로 응답하세요:
{
  ...
  "category": "적절한 카테고리명",
    ...
    const fallbackReviewImageIndices = hasAnyReviewImages
        ? pickFallbackReviewImageIndices(availableReviewsForImages, hasRichReviews
            ? pickReviewIndices(reviewCollection.reviews.length, articleVariation, 30, 8)
            : pickReviewIndices(product.reviews.length, articleVariation, 15, 5), 3)
        : [];
    const finalUsedReviewImageIndices = mergeReviewImageIndices(modelReviewImageIndices, promptReviewImageIndices, fallbackReviewImageIndices).slice(0, 5);
    ...
    const faqHtml = faqItems.length > 0
        ? `\n<div itemscope itemtype="https://schema.org/FAQPage">\n<h2>Q&A — 자주 묻는 질문</h2>\n${faqItems
            .map((f) => `<div itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">\n<h3 itemprop="name">${f.question}</h3>\n<div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">\n<p itemprop="text">${f.answer}</p>\n</div>\n</div>`)
            .join("\n")}\n</div>`
        : "";
...
// ── Route ────────────────────────────────────────────────────────────────────
export async function generateArticlesRoutes(app) {
    app.post("/generate-articles", async (req, reply) => {
        const { product, contentPrompt, siteConfigs, reviewCollection, offset = 0, limit, globalTotal, aeoConfig, contentStrategy, } = req.body;
        ...
        try {
            for (let i = 0; i < batchTasks.length; i += PARALLEL) {
                ...
                const results = await Promise.allSettled(parallelSlice.map(async ({ site, diversityIndex, siteArticleIndex }, sliceIdx) => {
                    ...
                    for (let attempt = 0; attempt < 5; attempt++) {
                        try {
                            ...
                            return await generateForSite(apiKey, product, contentPrompt, site, reviewCollection, diversityIndex, siteArticleIndex, aeoConfig, contentStrategy);
                        }
                        catch (err) {
                ...
                for (const result of results) {
                    if (result.status === "fulfilled") {
                        ...
                        send({ type: "error", message: result.reason.message?.slice(0, 150) ?? "생성 실패" });
                    }
                }
            }
            send({
...
//# sourceMappingURL=generate-articles.js.map