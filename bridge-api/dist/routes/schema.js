...
import { resolveSiteTarget, getSiteDirForTarget, isRemoteTarget, } from "../lib/server-targets.js";
...
function buildMuPluginPhp(schemaJson, siteSlug) {
...
async function checkSiteSchemaStatus(siteUrl) {
    try {
...
async function extractBusinessInfoWithAI(htmlContent, url) {
...
export async function schemaRoutes(app) {
            ...
            for (const site of sites) {
                try {
                    ...
                    send({
                        ...
                        hasLocalBusiness: status.schemaTypes.some((t) => t === "LocalBusiness" ||
                            t === "Restaurant" ||
                            t === "Dentist" ||
    ...
    app.post("/schema/install", async (req, reply) => {
        const { sites: siteSlugs, schemaJson } = req.body;
        if (!siteSlugs?.length) {
        ...
        try {
            ...
            const selectedSites = allSites.filter((s) => siteSlugs.includes(s.slug));
            ...
            for (const site of selectedSites) {
                ...
                try {
                    ...
                    if (isRemoteTarget(target)) {
                        ...
                        execSsh(target, `sudo mkdir -p ${shellQuote(muPluginDir)} && sudo tee ${shellQuote(pluginPath)} > /dev/null << 'EOFSCHEMA'\n${pluginContent}\nEOFSCHEMA\nsudo chown www-data:www-data ${shellQuote(pluginPath)} && sudo chmod 644 ${shellQuote(pluginPath)}`, 30000);
                    }
                    else {
