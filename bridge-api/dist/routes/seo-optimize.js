...
function buildSchemaBlocks(post, site, contentHtml) {
    let schemas = "";
...
function buildProductSchemaFromPost(post, site, contentHtml) {
    const plainTitle = stripHtml(post.title.rendered);
    // 가격 추출
...
// ── llms-full.txt ────────────────────────────────────────────────────────────
async function updateLlmsFullTxt(site, posts) {
...
// ── Route ────────────────────────────────────────────────────────────────────
export async function seoOptimizeRoutes(app) {
    app.post("/seo-optimize", async (req, reply) => {
        const { siteSlug, sites: sitesList } = req.body;
        ...
        try {
            for (const site of sites) {
                ...
                let totalPosts = 0;
                try {
                    const countRes = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=1&_fields=id&status=publish`, { headers, cache: "no-store" });
                ...
                for (let prefetchPage = 1; prefetchPage <= Math.ceil(totalPosts / PER_PAGE); prefetchPage++) {
                    try {
                        const res = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=${PER_PAGE}&page=${prefetchPage}&_fields=id,title,content,excerpt,link,date,modified,slug,categories,tags&status=publish`, { headers, cache: "no-store" });
                        if (!res.ok)
                            break;
                ...
                for (const post of allSitePosts) {
                    ...
                    const contentUnchanged = normalizeHtmlSignature(contentWithoutSchemas) === normalizeHtmlSignature(improvedContent);
...
//# sourceMappingURL=seo-optimize.js.map