...
import { buildBusinessSchemaFromHtml } from "../lib/business-schema.js";
 ...
 */
function applyAeoEnhancements(html, site, postTitle, postSlug, postCategory, postExcerpt, postLink) {
...
// ── Route ────────────────────────────────────────────────────────────────────
export async function backfillAeoRoutes(app) {
    app.post("/backfill-aeo", async (request, reply) => {
        ...
        try {
            ...
            while (allPosts.length < maxPosts) {
                const res = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=${perPage}&page=${page}&status=publish&_fields=id,title,content,slug,link,excerpt,categories`, { headers: { Authorization: authHeader }, signal: AbortSignal.timeout(REMOTE_FETCH_TIMEOUT_MS) });
                if (!res.ok)
                    break;
            ...
            for (const [idx, post] of allPosts.entries()) {
                try {
                    ...
                    const enhanced = applyAeoEnhancements(post.content.rendered, site, title, post.slug, categoryName, excerpt, post.link);
                    if (dryRun) {
                        updatedCount++;
                        send({
                            type: "preview",
...
//# sourceMappingURL=backfill-aeo.js.map