...
const DEPLOY_SCRIPT = process.env.DEPLOY_SCRIPT_PATH || "/home/ubuntu/wp-bulk-generator/scripts/deploy-wp-sites.sh";
const SECONDARY_PROXY_SYNC_SCRIPT = process.env.SECONDARY_PROXY_SYNC_SCRIPT ||
    join(getPrimaryServerTarget().repoRoot, "scripts", "sync-secondary-proxies.sh");
const MIN_BATCH_FREE_KB_HEADROOM = Number(process.env.MIN_BATCH_FREE_KB_HEADROOM || 524288);
const ESTIMATED_SITE_DISK_KB = Number(process.env.ESTIMATED_SITE_DISK_KB || 153600);
const DEPLOY_CACHE_WARM_TIMEOUT_MS = Number(process.env.DEPLOY_CACHE_WARM_TIMEOUT_MS || 12000);
const DEPLOY_CACHE_WARM_CONCURRENCY = Math.max(1, Number(process.env.DEPLOY_CACHE_WARM_CONCURRENCY || 4));
const DEPLOY_BING_SYNC_CONCURRENCY = Math.max(1, Number(process.env.DEPLOY_BING_SYNC_CONCURRENCY || 3));
...
function findDuplicates(configs) {
...
function readExistingSites() {
    try {
        ...
        const raw = execSync(`sudo test -f ${shellQuote(CREDS_PATH)} && sudo cat ${shellQuote(CREDS_PATH)} || printf "[]"`, { timeout: 10000 }).toString();
...
function readExistingConfigs() {
    try {
        ...
        const raw = execSync(`sudo test -f ${shellQuote(CONFIG_PATH)} && sudo cat ${shellQuote(CONFIG_PATH)} || printf "[]"`, { timeout: 10000 }).toString();
        return JSON.parse(raw);
    }
    catch {
        return [];
...
function parseFreeDiskKb(raw) {
    const value = Number(String(raw).trim());
    return Number.isFinite(value) ? value : 0;
}
function getTargetFreeDiskKb(target) {
...
function estimateRequiredDiskKb(siteCount) {
    return MIN_BATCH_FREE_KB_HEADROOM + siteCount * ESTIMATED_SITE_DISK_KB;
}
function pickDeployTarget(siteCount) {
    ...
    if (isRemoteTarget(preferred) && preferred.id !== primary.id) {
        ...
        if (primaryFreeKb >= requiredKb) {
            return {
                target: primary,
                message: `기본 배포 서버(${preferred.host}) 디스크가 부족해 기존 서버로 우회합니다. ` +
    ...
    throw new Error(`배포 서버 디스크 공간이 부족합니다. ` +
        `(필요 약 ${Math.ceil(requiredKb / 1024)}MB, ` +
        `${preferred.id}${preferred.host ? ` ${preferred.host}` : ""} 여유 약 ${Math.floor(preferredFreeKb / 1024)}MB)`);
}
function getCredentialKey(site) {
    return normalizeSlug(site.site_slug) || normalizeSlug(site.slug) || normalizeDomain(site.domain);
}
...
function filterMatchedCredentials(credentials, configs) {
    const requestedSlugs = new Set(configs.map((cfg) => normalizeSlug(cfg.site_slug)).filter(Boolean));
    const requestedDomains = new Set(configs.map((cfg) => normalizeDomain(cfg.domain)).filter(Boolean));
...
function applyTargetMetadata(credentials, target) {
    if (!isRemoteTarget(target)) {
        return credentials;
    }
...
function buildSiteWarmTargets(siteUrl) {
...
async function warmDeployedSites(sites) {
    const urls = Array.from(new Set(sites.flatMap((site) => buildSiteWarmTargets(site.url)).filter(Boolean)));
    let cursor = 0;
    const workers = Array.from({ length: Math.min(DEPLOY_CACHE_WARM_CONCURRENCY, urls.length) }, async () => {
...
async function syncDeployedSitesWithBing(sites, onLog) {
    if (!isBingWebmasterSyncEnabled()) {
        return;
    }
    const urls = Array.from(new Set(sites
    ...
    let feedCount = 0;
    let errorCount = 0;
    const workers = Array.from({ length: Math.min(DEPLOY_BING_SYNC_CONCURRENCY, urls.length) }, async () => {
        while (cursor < urls.length) {
            const index = cursor++;
            const url = urls[index];
            try {
                ...
                if (result.errors.length > 0) {
                    errorCount += 1;
                    onLog(`Bing sync partial: ${result.siteUrl} (add=${result.added ? "ok" : "fail"}, sitemap=${result.feedSubmitted ? "ok" : "fail"})`);
                ...
                errorCount += 1;
                onLog(`Bing sync failed: ${url} - ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    });
    await Promise.allSettled(workers);
    onLog(`Bing sync summary: ${urls.length}개 사이트, add ${addedCount}건, sitemap ${feedCount}건, 오류 ${errorCount}건`);
}
function syncLocalCaches(credentials, configs) {
    const primary = getPrimaryServerTarget();
...
function readTargetCredentials(target) {
    try {
        if (!isRemoteTarget(target)) {
            return readExistingSites();
        }
        const raw = execSsh(target, `sudo cat ${shellQuote("/root/wp-sites-credentials.json")}`, 20000);
        return JSON.parse(raw);
    }
    catch {
        return [];
...
function createDeployProcess(target, configs) {
    ...
    const remoteConfigPath = execSsh(target, "mktemp /tmp/sites-config-deploy-XXXXXX.json", 15000);
...
function parseDeployMarker(line) {
...
function summarizeDeployCredentials(credentials, configs) {
    const requestedSlugs = new Set(configs.map((cfg) => normalizeSlug(cfg.site_slug)).filter(Boolean));
    const requestedDomains = new Set(configs.map((cfg) => normalizeDomain(cfg.domain)).filter(Boolean));
    const matched = credentials.filter((site) => {
...
function appendChunkLines(pending, data, onLine) {
...
export async function deployRoutes(app) {
    app.post("/deploy", async (req, reply) => {
        ...
        let deployCleanup = () => undefined;
        try {
            let completed = 0;
            let successCount = 0;
            let failureCount = 0;
            const successfulSlugs = new Set();
            const failedSites = conflicts.map((slug) => ({
                slug,
            ...
            const { target: deployTarget, message: targetSelectionMessage } = pickDeployTarget(deployableConfigs.length);
            const configsToPersist = deployableConfigs.map((config) => isRemoteTarget(deployTarget)
            ...
            await new Promise((resolve, reject) => {
                child.on("close", (code) => {
                    ...
                    if (code === 0)
                        ...
                        reject(new Error(`배포 스크립트 종료 코드: ${code}`));
                });
            ...
            const matchedCredentials = applyTargetMetadata(filterMatchedCredentials(targetCredentials, deployableConfigs), deployTarget);
            const mergedCredentials = mergeCredentials(readExistingSites(), matchedCredentials);
            ...
            const credentialsSummary = summarizeDeployCredentials(mergedCredentials, deployableConfigs);
