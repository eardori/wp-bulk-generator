...
function normalizePublishedPostUrl(rawUrl, site, fallbackSlug, fallbackPostId) {
...
function buildDashboardPostEntry(article, result) {
    return {
        id: result.postId,
...
async function uploadToWordPress(buffer, filename, contentType, site) {
    const baseUrl = getSiteBaseUrl(site);
...
async function uploadReviewImages(images, site, send) {
...
function replacePlaceholders(html, article, uploadedMap, _reviewCollection) {
...
async function probeRemoteWordPress(site) {
    const baseUrl = getSiteBaseUrl(site);
    try {
...
function buildFinalHtml(article, site, replacedHtml) {
    const baseUrl = getSiteBaseUrl(site);
 ...
 */
function buildAuthorBioHtml(site) {
    ...
    const infoLines = [];
    if (expertise)
        infoLines.push(`<strong>전문 분야</strong>: ${expertise}`);
    if (concern)
        infoLines.push(`<strong>관심사</strong>: ${concern}`);
 ...
 */
function buildHowToSchema(html, articleTitle) {
    const $ = cheerio.load(html, null, false);
    let howToSteps = [];
    let howToName = "";
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    $("h2").each((_, h2El) => {
        ...
        if (!/(사용법|사용 방법|사용 순서|이용 방법|사용 팁|활용법|how\s*to)/i.test(h2Text))
            return;
        howToName = h2Text;
 ...
 */
function enhanceFaqMicrodata(html) {
...
function buildProductReviewSchema(article, site) {
...
async function fetchRelatedPosts(site, currentSlug, category, maxPosts = 3) {
    const baseUrl = getSiteBaseUrl(site);
    const authHeader = "Basic " + Buffer.from(`${site.admin_user}:${site.app_pass}`).toString("base64");
    try {
        // 같은 카테고리의 최근 글 가져오기
        const res = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=${maxPosts + 1}&_fields=id,title,link,slug&status=publish&orderby=date&order=desc`, { headers: { Authorization: authHeader }, signal: AbortSignal.timeout(10000) });
        if (!res.ok)
            return [];
...
async function publishViaRestApi(article, site, finalHtml) {
    ...
    for (const tagName of tagNames) {
        try {
            const tagRes = await fetch(`${baseUrl}/wp-json/wp/v2/tags?search=${encodeURIComponent(tagName)}`, { headers: wpHeaders, signal: AbortSignal.timeout(REMOTE_FETCH_TIMEOUT_MS) });
...
function resolvePublishedPostBySlug(site, slug) {
    const target = resolveSiteTarget(site);
    const siteDir = getLocalSiteDir(site);
    const php = [
    ...
    try {
        const output = !isRemoteTarget(target)
            ...
            : execSsh(target, `wp eval ${shellQuote(php)} --path=${shellQuote(siteDir)} --allow-root`, 60000).trim();
...
async function publishLocallyWithWpCli(article, site, finalHtml) {
    const target = resolveSiteTarget(site);
    const siteDir = getLocalSiteDir(site);
...
`);
        let output = "";
        if (!isRemoteTarget(target)) {
            output = execFileSync("wp", ["eval-file", scriptPath, `--path=${siteDir}`, "--allow-root"], { encoding: "utf8", timeout: 45000 }).trim();
        }
        else {
            ...
            output = execSsh(target, `wp eval-file ${shellQuote(remoteScriptPath)} --path=${shellQuote(siteDir)} --allow-root`, 60000).trim();
            try {
...
// ── WordPress publish ────────────────────────────────────────────────────────
async function publishToWordPress(article, site, send) {
...
async function updateLlmsTxt(site) {
    const baseUrl = getSiteBaseUrl(site);
    const authHeader = "Basic " + Buffer.from(`${site.admin_user}:${site.app_pass}`).toString("base64");
    try {
        // 최근 글 50개 가져오기
        const res = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=50&_fields=title,link,excerpt&status=publish&orderby=date&order=desc`, { headers: { Authorization: authHeader }, signal: AbortSignal.timeout(15000) });
        if (!res.ok)
            return;
        const posts = await res.json();
...
`;
        ...
        if (isRemoteTarget(target)) {
            ...
            try {
                ...
                execSsh(target, `sudo cp ${shellQuote(`${remoteTempDir}/llms.txt`)} ${shellQuote(`${siteDir}/llms.txt`)} && sudo cp ${shellQuote(`${remoteTempDir}/llms-full.txt`)} ${shellQuote(`${siteDir}/llms-full.txt`)} && sudo chown www-data:www-data ${shellQuote(`${siteDir}/llms.txt`)} ${shellQuote(`${siteDir}/llms-full.txt`)}`, 30000);
            }
            finally {
                try {
                    execSsh(target, `rm -rf ${shellQuote(remoteTempDir)}`, 15000);
...
async function warmPublishedUrls(baseUrl, postUrl) {
...
async function deployIndexNowKeyFile(site) {
    ...
    try {
        ...
        if (isRemoteTarget(target)) {
            ...
            try {
                ...
                execSsh(target, `sudo cp ${shellQuote(remoteTmpPath)} ${shellQuote(`${siteDir}/${keyFileName}`)} && sudo chown www-data:www-data ${shellQuote(`${siteDir}/${keyFileName}`)} && rm -f ${shellQuote(remoteTmpPath)}`, 15000);
            }
            finally {
                rmSync(tempDir, { recursive: true, force: true });
            }
            ...
            for (let i = 0; i < articles.length; i++) {
                ...
                try {
                    ...
                    await updateDashboardSiteCache(article.siteSlug, (current) => {
                        const existingPosts = current.posts.filter((post) => post.id !== publishedPost.id && post.link !== publishedPost.link);
                        const nextPosts = [publishedPost, ...existingPosts].slice(0, 15);
                        const nextTotalCount = current.posts.some((post) => post.id === publishedPost.id || post.link === publishedPost.link)
                            ? current.totalCount
