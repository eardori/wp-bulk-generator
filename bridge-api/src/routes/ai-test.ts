...
async function callGemini(query: string): Promise<{ text: string; model: string }> {
...
async function callOpenAI(query: string): Promise<{ text: string; model: string }> {
...
async function callAnthropic(query: string): Promise<{ text: string; model: string }> {
...
function analyzeResponse(
  response: string,
  businessName: string
): Omit<PlatformResult, "platform" | "fullResponse" | "modelVersion" | "error"> {
  ...
  if (mentioned) {
    ...
    if (positiveWords.some((w) => contextLower.includes(w))) sentiment = "positive";
    else if (negativeWords.some((w) => contextLower.includes(w))) sentiment = "negative";
...
type PlatformConfig = {
  id: string;
  label: string;
  callFn: (query: string) => Promise<{ text: string; model: string }>;
};
...
export async function aiTestRoutes(app: FastifyInstance) {
  // ─── POST /ai-test/generate-queries ────────────────────────
  // 업종/지역 기반 쿼리 자동 생성
  app.post("/ai-test/generate-queries", async (req) => {
    const body = req.body as QueryGenerateRequest;
  ...
  app.post("/ai-test/run", async (req, reply) => {
    ...
    if (platforms.length === 0) {
      return reply.status(400).send({
        error:
          "사용 가능한 AI 플랫폼이 없습니다. .env에 OPENAI_API_KEY, GEMINI_API_KEY, ANTHROPIC_API_KEY 중 하나 이상을 설정해주세요.",
      });
    }
    ...
    try {
      ...
      for (const platform of platforms) {
        citationRates[platform.id] = Math.round(
          (citationCounts[platform.id] / enabledQueries.length) * 100
        );
      }
      const avg = Math.round(
        Object.values(citationRates).reduce((a, b) => a + b, 0) / platforms.length
      );
